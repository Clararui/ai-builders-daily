...
for(const [index,item] of selection.items.entries()) {
  ...
  const summary=cleanSummary(run.stdout,item.text);
  results.push({...item,summary});
  console.log('Draft item '+(index+1)+'/'+selection.items.length+' complete');
}
await writeFile(resolve(output,'draft.json'),JSON.stringify({generatedAt:new Date().toISOString(),sourceUpdatedAt:selection.sourceUpdatedAt,status:'draft',humanReviewRequired:true,items:results},null,2));
